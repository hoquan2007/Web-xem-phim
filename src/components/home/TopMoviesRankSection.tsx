...
export const TopMoviesRankSection: React.FC<TopMoviesRankSectionProps> = ({ movies }) => {
  ...
  return (
    <div className="w-full bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-slate-900/90 border border-slate-800/80 rounded-2xl p-4 sm:p-6 backdrop-blur-md shadow-2xl relative overflow-hidden">
      ...
      <div
        ref={scrollRef}
        className="flex items-center gap-5 sm:gap-6 overflow-x-auto scrollbar-none py-2 relative z-10 scroll-smooth snap-x snap-mandatory"
        ...
        {top10.map((movie, index) => {
          ...
          return (
            <Link
              ...
              className="flex-shrink-0 flex items-center group relative w-[240px] sm:w-[280px] snap-start bg-slate-950/60 hover:bg-slate-800/60 p-2.5 rounded-xl border border-slate-800/60 hover:border-amber-400/40 transition-all duration-300 shadow-lg hover:shadow-amber-500/10"
            >
              {/* Giant Rank Number */}
              <div className="w-16 sm:w-20 flex items-center justify-center font-black select-none leading-none">
                <span
                  className={`text-5xl sm:text-6xl font-extrabold tracking-tighter ${
                    rank === 1
                      ? 'text-transparent bg-clip-text bg-gradient-to-b from-amber-300 via-amber-400 to-amber-600 drop-shadow-[0_0_12px_rgba(245,158,11,0.5)]'
                      : rank === 2
                      ? 'text-transparent bg-clip-text bg-gradient-to-b from-slate-200 via-slate-300 to-slate-500'
                      : rank === 3
              ...
              {/* Movie Poster */}
              <div className="relative w-20 sm:w-24 aspect-[2/3] rounded-lg overflow-hidden flex-shrink-0 shadow-md">
                <Image
                  src={posterSrc}
                  alt={movie.name}
                  fill
                ...
                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-slate-950/40 backdrop-blur-[2px]">
                  <div className="w-9 h-9 rounded-full bg-amber-400 text-slate-950 flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform">
              ...
              <div className="ml-3 flex-1 min-w-0">
                <h3 className="text-sm font-bold text-slate-100 group-hover:text-amber-400 transition-colors truncate leading-snug">
                  {movie.name}
                </h3>
                ...
                <div className="flex flex-wrap items-center gap-1.5 mt-2">
                  ...
                  {movie.episode_current && (
                    <span className="px-1.5 py-0.5 text-[10px] font-medium bg-slate-800 text-slate-300 rounded truncate max-w-[100px]">
                      {movie.episode_current}
                    </span>
                  )}
                </div>
