...
| **FIX-20** | Loại bỏ VSMOV provider + nâng cấp Ophim thành full catalogue provider (thay thế VSMOV bằng 1 API thực sự hoạt động) | ✅ Completed | Diagnose 2026-08-07 dùng PowerShell `Invoke-WebRequest` probe VSMOV upstream: `/api/danh-sach/phim-moi-cap-nhat` + `/api/tim-kiem` + `/api/phim/<vsmov-slug>` đều **200 JSON hợp lệ**, nhưng `/api/phim/<kkphim-slug>` luôn **404 Not Found** vì VSMOV dùng slug format khác (`avengers-4-hoi-ket-178544376755563` vs KKPhim `avengers-endgame`). Adapter `vsmovAdapter` nhận slug từ KKPhim primary → 404 → orchestrator log "timeout" sai (FIX-16 đã vá nhưng root cause vẫn còn). Probe thêm: hostname `image.vsmov.com` whitelisted trong `next.config.ts` đã chết (DNS NXDOMAIN) → ảnh phim (nếu có) cũng không load được. Probe 3 ứng viên thay thế: (1) **Ophim** đầy đủ 5 endpoint (list/search/cat/ctr/detail) với schema `{ status, data: { items, params: { pagination } } }` y hệt KKPhim v1 → chọn Ophim; (2) KKPhim alternate `kkphim.com` DNS fail; (3) TMDB/OMDb cần key (không phù hợp free-tier). Fix (a) `src/lib/api/adapters.ts:54-66` — `PROVIDER_ENABLED.vsmov = false` cứng, xoá `vsmovAdapter` factory (60 dòng), xoá `VSMOV_BASE`/`VSMOV_TIMEOUT` exports. (b) `src/lib/api/adapters.ts:323-477` — nâng cấp `ophimAdapter` thành full catalogue provider: implement `list` (build URL theo `category`/`country`/`type` qua `OPHIM_TYPE_MAP`), `search` (`/v1/api/tim-kiem`), `categories` (`/v1/api/the-loai`), `countries` (`/v1/api/quoc-gia`), `detail` (giữ nguyên). Thêm `OPHIM_CDN` env var (`API_CDN_OPHIM` → `image.ophim1.com`) để absolute URL image working. (c) `src/lib/api/adapters.ts:597` — `getEnabledAdapters()` rút gọn `[kkphim, ophim, nguonc]` (3 thay vì 4). (d) `src/lib/__fixtures__/provider-fixtures.ts` — xoá `fixtureVsmovDetail`, thêm `fixtureOphimList(page, limit)` + `fixtureOphimSearch(keyword)`. (e) `src/lib/api/mock-handler.ts` — thêm `ophimListRoute`/`ophimSearchRoute`/`ophimCategoriesRoute`/`ophimCountriesRoute` cho Ophim catalogue, stub `vsmovDetailRoute` trả 410 Gone cho stale E2E. (f) `src/components/ui/SafeImage.tsx:53-65` — xoá `image.vsmov.com` khỏi `CDN_BYPASS_OPTIMIZER` (host chết). (g) `next.config.ts:49-62` — xoá whitelist `image.vsmov.com`, thêm comment FIX-18. (h) `playwright.config.ts` — xoá `API_BASE_VSMOV` khỏi `mockEnv`, đổi comment KKPhim-disable project. (i) `.env.example` — xoá `API_DISABLE_VSMOV` + `API_BASE_VSMOV`, thêm `API_CDN_OPHIM`. (j) `src/lib/api.ts:43-49, 351-359` + `src/lib/api/providers.ts:9-15, 446-450` + `src/components/watch/WatchContainer.tsx:74` — cập nhật doc comments. (k) `tests/e2e/disable-flag.spec.ts:11-50, 56-66` — test fallback chain bỏ VSMOV, expect Ophim catalogue thay thế. (l) `scripts/test-api.ts` + `scripts/test-disable-flag.ts` — bỏ reference `vsmovAdapter`/`fixtureVsmovDetail`, dùng stub adapter. (m) `scripts/test-mock.ts` — thay 2 VSMOV test bằng test `410 Gone` + 4 Ophim catalogue test. (n) `scripts/probe-providers.ts` — bỏ `vsmov` khỏi `ProviderId`, bỏ definition VSMOV detail. (o) `docs/provider-scorecard.md` — đánh dấu VSMOV "đã loại bỏ", nâng Ophim lên "FIX-18: full catalogue provider (TBD re-probe)". (p) `src/app/api/mock/[...path]/route.ts:5-12` + `src/components/ui/SafeImage.tsx:55` + các comment cũ — sửa doc. Verify: `npx tsc --noEmit` 0 lỗi; `npm run lint` 0 errors; `npm run build` 9/9 prerender; `npm run test:unit` **200 passed** (51 sanitize + 61 validate + 57 api + 31 disable-flag); `npm run test:mock` 23/23 pass (4 Ophim catalogue + 1 vsmov 410 + 18 cũ regression). **Lưu ý vận hành:** slug của Ophim khác KKPhim (`avengers-hoi-ket` vs `avengers-endgame`) → cross-provider detail lookup sẽ 404 → orchestrator log warning + fallback sang KKPhim metadata (degraded=true). Catalogue cross-provider chạy bình thường vì mỗi provider tự list theo slug của riêng họ. |
